...
router.post('/livreur/signin', async (req, res, next) => {
  const { email, password } = req.body;

  try {
    // Retrieve delivery agent data from database
    const livreur = await Livreur.findOne({ email: email });
    ...
    const passwordMatched = await bcrypt.compare(password, livreur.password);
    if (!passwordMatched) {
      res.status(401).json({ message: 'Invalid password' });
      return;
    }
    ...
    res.status(200).send({TOKEN_SECRET_KEY, user: 'livreur' })
